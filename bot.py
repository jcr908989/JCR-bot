...
# ============================================================
# 🎨 ESTILOS MÍNIMOS (el tema oscuro lo maneja Streamlit)
# ============================================================
st.markdown("""
<style>
...
# ============================================================
def init_db():
    conn = sqlite3.connect("jcr_bot_v4.db", check_same_thread=False)
    c = conn.cursor()
    
    # Tabla de historial
    c.execute("""CREATE TABLE IF NOT EXISTS historial 
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  timestamp TEXT, url TEXT, tienda TEXT, 
                  estado TEXT, precio TEXT, precio_anterior TEXT,
                  cambio_precio TEXT)""")
    
    # Tabla de precios actuales
    c.execute("""CREATE TABLE IF NOT EXISTS precios_actuales
                 (url TEXT PRIMARY KEY, precio TEXT, timestamp TEXT)""")
    
    # Tabla de URLs vigiladas
    c.execute("""CREATE TABLE IF NOT EXISTS urls_vigiladas
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  url TEXT UNIQUE,
                  nombre TEXT,
...
def agregar_url(conn, url, nombre=None):
    try:
        ...
        c.execute("""INSERT INTO urls_vigiladas (url, nombre, activa, fecha_agregada) 
                     VALUES (?, ?, 1, ?)""",
